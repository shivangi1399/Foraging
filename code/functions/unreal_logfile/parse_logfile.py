...
with TextLog('/mnt/gs/departmentN5/preprocessing/example_data/2021_01_14-14_59_35_GrassyLandscape_Cont.log') as log:
...
""" This module allows the logfile to be parsed
...
class TextLog:
    ...
    def read_log_header_with_return(self):
        i = 0
        header = dict()
    ...
    def create_file_matching_lines(self, search_string, st_line=0, nlines=1, st=0, end=-1):
        search_bytes = bytes(search_string, self.enc)

        lines = []
        with NamedTemporaryFile("wb",dir=".", delete=False) as temp:
            reading = True
            while reading:
                ...
                for iline in range(nlines+st_line):
                    line = self.mm.readline()
                    if iline >= st_line:
                        temp.write(line)
                st = self.mm.tell()
    ...
    def count_coincidences(self, search_string, st_line=0, nlines=1, st=0, end=-1):
        search_bytes = bytes(search_string, self.enc)
        ...
        while reading:
            ...
            if ind == -1:
                reading = False
                break
            counterCoincidences += 1
            st = ind + len(search_string)#self.mm.tell()
    ...
    #def find_obj_timestamps(self):
        # if we grabbed the start of each object reference that might help to find just that object
        # most objects only live for the length of a trial
        # it should be the spawn time of every object
    ...
    def parse_stateid_times(self, state_id, st, end, times='StateStarted', return_index=False, convert=True):
        ...
        #search_bytes =  bytes('['+istate+'],LogTypes::'+times, self.enc)  #[150360],LogTypes::StateStarted
    ...
    # def parse_state_changes(self, state_id, st, end):
    #     zero_buf = 100
    ...
    def parse_parameters(self, obj_id=None, st=0, end =-1, obj=None):
        ...
        # if end == -1:
        #     # This is not safe, doesn't check if it's the only id
        #     self.all_ids[self.all_ids['id'] == obj_id]['end'] = st
    ...
    # def get_info_per_trial(self, return_spawned=True, spawned_name = 'ImageStimulus', return_param = ['ObjectName', 'Image', 'StimulusCategory'], return_loc = False, return_states=False, return_eventmarkers=False):

    #     st_ts, st_ind = self.parse_all_state_times(state='StartTrial', times='StateStarted', return_index=True)
    #     end_ts, end_ind = self.parse_all_state_times(state='EndTrial', times='StateStarted', return_index=True)

    #     # n_ts = sum([len(x) for x in ts])
    #     # st_ind = np.zeros((n_ts), dtype=int)
    #     # ii = 0
    #     # for times in ts:
    #     #     for itime in times:
    #     #         ts_bytes = b'\n%0.6f,['%(itime) # time stored to 6 decimal places
    #     #         st_ind[ii] = self.mm.find(ts_bytes)

    #     #         ii += 1

    #     all_trials = []
    #     for iblock, index in enumerate(st_ind):
    #         block_trials = []
    #         for iseg, st in enumerate(index):
    #             if iseg >= end_ind[iblock].size:
    #                 # more start trials than end trials
    #                 continue
    #             end = end_ind[iblock][iseg]

    #             trl_ids = self.get_ids_per_segment(st=st, end=end)
    #             trl_states = trl_ids[trl_ids["id_type"] == self.StateName]
    #             trl_obj = trl_ids[trl_ids["id_type"] == self.ObjectName]

    #             # create trial dictionary
    #             trial = dict()
    #             trial['StartTime'] = st_ts[iblock][iseg]
    #             trial['EndTime'] = end_ts[iblock][iseg]

    #             # find correct/ wrong
    #             trial['Outcome'] = 0
    #             trial['ReactionTime'] = np.nan
    #             #rt_ts = np.nan
    #             #task_ts = np.nan

    #             for i_id in trl_states:
    #                 name = i_id['name']
    #                 if 'Correct' in name or 'ErrorWrong' in name:
    #                     state_ts = self.parse_stateid_times(i_id['id'], st, end, times='StateStarted')
    #                     if state_ts.size > 0:
    #                         #.append(name)
    #                         trial['ReactionTime'] = state_ts[0] - trial['StartTime']
    #                         if 'Correct' in name:
    #                             trial['Outcome'] = 1

    #             # parsing objects
    #             if return_spawned:
    #                 trial['SpawnedStimuli'] = []
    #                 for ii, i_id in enumerate(trl_obj):
    #                     if spawned_name in i_id['name']:
    #                         obj_id = i_id['id']
    #                         spawn_st = i_id['start']
    #                         if spawn_st >= st and spawn_st < end:
    #                             tmp = self.parse_initial_parameters(obj_id=obj_id, st=spawn_st, end=end)
    #                             stim = dict(zip(return_param, [None]*len(return_param)))
    #                             for param in return_param:
    #                                 if param in stim:
    #                                     stim[param] = tmp[param]
    #                             trial['SpawnedStimuli'].append(stim)

    #             if return_loc:
    #                 trial['StimulusLocations'] = []
    #                 for i_id in trl_obj:
    #                     trial['StimulusLocations'].append(self.parse_position(obj_id=i_id['id'], st=st, end=end))

    #             if return_states:
    #                 trial['States'] = []
    #                 for i_id in trl_states:
    #                     trial['States'].append(self.parse_state_timings(state_id=i_id['id'], st=st, end=end))

    #             if return_eventmarkers:
    #                 evt, evt_ts, evt_desc, true_ts = self.parse_eventmarkers(st=st, end=end, zero_buff=100)
    #                 trial['Event'] = evt
    #                 trial['EventTime'] = evt_ts
    #                 trial['EventDesc'] = evt_desc
    #             block_trials.append(trial)
    #         all_trials.append(block_trials)
    #     return all_trials
    
    def select_trials(self, trial_info, conditions=None):
        """
        Selects trials based on given conditions.
        trial_info: a dictionary containing trial data
        conditions: a list of conditions to apply. Each condition is a string that matches a key in the 
                 trial_info dictionary. Accepts only arrays 1/0 or True/False.  
        Returns: a boolean array representing selected trials. The length of the array is equal to the number 
             of trials, and a True value at a given index indicates that the corresponding trial 

        """

        ntrials = len(trial_info['StartTs'])

        # If conditions are not provided or the list is empty
        if not conditions:
            return np.full(ntrials, True, dtype=bool)
        
        conditions_results = []

        for condition in conditions:
            if '=' in condition:
                trials = np.full(ntrials, False, dtype=bool)
                name = condition.split('=')[0]
                if '~' in name:
                    name = name.split('~')[-1]
                value = condition.split('=')[-1]
                trials[np.where(trial_info[name].astype(str) == value)[0]] = True
                if '~' in condition:
                    trials = ~trials
            elif '~' in condition:
                name = condition.split('~')[-1]
                trials = ~trial_info[name].astype(bool)
            else:
                trials = trial_info[condition].astype(bool)
            conditions_results.append(trials)

        chosen_trl = np.logical_and.reduce(conditions_results)
   
        return chosen_trl


    def get_info_per_trial(self, return_eventmarkers=False, choose_trials=False, return_loc = False, 
                        start=3000, end=3090, return_spawned=False, spawned_name = 'ImageStimulus', 
                        return_param = ['ObjectName', 'Image', 'StimulusCategory'], return_states=False):
        """
        Returns detailed trial information based on specified criteria and parameters.
        
        To select specific trials: choose_trials=['Key']. This will give you all the trials that are True for this key, e.g. 'Correct'. 
        Use '~Key' to select the inverse trials and 'Key=X' to select trials with specified values, e.g. 'BaseBlock=4'.
        To choose alternative start or end of the trial: start=eventmarker, e.g. start=3042.
        """

        # Initiate dictionary to store trial info
        trial_info=dict()
        
        # Extract eventmarkers
        evt, tse, evt_desc, QPC=self.parse_eventmarkers()

       # ---------------- break eventmarkers into trials ----------------

        start_trial=3000 # snippet from start trial to start trial eventmarker

        starts=np.where(evt==start_trial)[0][:-1]
        ends=np.where(evt==start_trial)[0][1:]

        blocks=np.where((10<=evt)&(evt<50))[0]
        
        if len(starts)>len(ends):
            starts=starts[0:-1]

        trial_info['StartTs']=tse[starts]
        trial_info['EndTs']=tse[ends]
        trial_info['FlashTs']=QPC[starts]

        tr_starts_ts=trial_info['StartTs']

        ntrials=len(starts)

        evt_tr=np.empty(ntrials, dtype=object)
        evt_ts_tr=np.empty(ntrials, dtype=object)
        evt_desc_tr=np.empty(ntrials, dtype=object)

        trial_info['BaseBlock']=np.zeros(ntrials,dtype=int)
        
        if len(blocks)!=0:
            ibl=np.argmin(abs(blocks-starts[0]))
            
        for itrl in range(ntrials):

            trial=evt[starts[itrl]:ends[itrl]+1]
            
            if len(blocks)!=0:
                if ibl<len(blocks)-1:
                    if blocks[ibl] < starts[itrl] and starts[itrl] < blocks[ibl+1]:
                        block=evt[blocks[ibl]]
                        trial_info['BaseBlock'][itrl]=block-10
                    else:
                        block=evt[blocks[ibl+1]]
                        trial_info['BaseBlock'][itrl]=block-10
                        ibl+=1
                else:
                    block=evt[blocks[ibl]]
            ...
            evt_tr[itrl]=trial
            evt_ts_tr[itrl]=tse[starts[itrl]:ends[itrl]+1]
            evt_desc_tr[itrl]=evt_desc[starts[itrl]:ends[itrl]+1]

        if return_eventmarkers:
            trial_info['Event'] = evt_tr
            trial_info['EventTs'] = evt_ts_tr
            trial_info['EventDesc'] = evt_desc_tr


        # ---------------- basic info per trial  ----------------
        
        for key in (['TrialIndex','Correct','Wrong','Error','PlaceholderDistance','Distance','Offset','Left','Right','BaseConditionTarget','BaseConditionDistractor','MorphTarget','MorphDistractor']):

            trial_info[key]=np.zeros(ntrials,dtype=int)
            new_starts=np.full(ntrials, False, dtype=bool)
            new_starts_ts=np.zeros(ntrials,dtype=float)
            new_ends=np.full(ntrials, False, dtype=bool)
            new_ends_ts=np.zeros(ntrials,dtype=float)
        
        for itrl,trl in enumerate(evt_tr):
            trial_info['TrialIndex'][itrl]=itrl
            countCond=0
            countMorph=0
            for imark,marker in enumerate(trl):                
                if marker==1:
                    trial_info['Correct'][itrl]=1
                elif marker==2:
                    trial_info['Wrong'][itrl]=1

                elif 100<marker<140 and 3<=int(repr(marker)[-1])<=7:
                    trial_info['Error'][itrl]=marker-100

                elif 50<=marker<100:
                    if countCond == 0:
                        trial_info['BaseConditionTarget'][itrl]=marker-50
                        countCond = 1
                    else:
                        trial_info['BaseConditionDistractor'][itrl]=marker-50

                elif 3400<=marker<=3500:
                    if countMorph == 0:
                        trial_info['MorphTarget'][itrl]=(marker-3400)
                        countMorph=1
                    else:
                        trial_info['MorphDistractor'][itrl]=(marker-3400)

                if start !=3000 or end!=3000:
                    if marker == start and not new_starts[itrl]:
                        new_starts[itrl] = 1
                        new_starts_ts[itrl] = evt_ts_tr[itrl][imark]
                        new_starts[itrl] = True  
                    elif marker == end and not new_ends[itrl]:
                        new_ends[itrl] = 1
                        new_ends_ts[itrl] = evt_ts_tr[itrl][imark]
                        new_ends[itrl] = True  

        # ---------------- find distance to stimuli ----------------
        for itrl, trl in enumerate(evt_tr):
            for imark,marker in enumerate(trl):
                if marker == 3006:
                    trial_info['PlaceholderDistance'][itrl]=trl[imark+1]-10000
                if marker == 3101:
                    trial_info['Distance'][itrl]=trl[imark+2]-10000
                    lr=trl[imark+3]
                    if int(repr(lr)[-1])==4:
                        trial_info['Offset'][itrl]=trl[imark+4]-10000
                        trial_info['Right'][itrl]=1  
                    else:
                        trial_info['Offset'][itrl]=-(trl[imark+4]-10000)
                        trial_info['Left'][itrl]=1  

        # ---------------- select trials that satisfy conditions ----------------
        chosen_trl = self.select_trials(trial_info, choose_trials)

        if start !=3000 or end!=3000:

            start_and_end=np.logical_and(new_starts,new_ends)
            chosen_trl=np.logical_and(chosen_trl,start_and_end)

            trial_info['StartTs']=new_starts_ts
            trial_info['EndTs']=new_ends_ts
        
        for key in trial_info:
            trial_info[key]=trial_info[key][chosen_trl] 
                
        tr_starts_ts=tr_starts_ts[chosen_trl]


        # ---------------- get character location in the environment (per trial) ----------------
        if return_loc:

            self.make_id_struct()
            
            for i, name in enumerate (self.all_ids['name']):
                if name.startswith('AnimalCharacter'):
                    AC = name
                    break

            loc, rot, ts =self.parse_position(obj=AC)
            
            trial_info['Location']=np.empty(len(trial_info['StartTs']), dtype=object)
            trial_info['LocationTs']=np.empty(len(trial_info['StartTs']), dtype=object)

            for itrl,trl in enumerate(trial_info['StartTs']):
                startloc=np.where(ts == trl)[0][0]
                starttrloc=np.where(ts==tr_starts_ts[itrl])[0][0]
                endloc=np.where(ts == trial_info['EndTs'][itrl])[0][0]
                trial_info['Location'][itrl]=loc[startloc:endloc,0:2] - loc[starttrloc,0:2]
                trial_info['LocationTs'][itrl]=ts[startloc:endloc]

        # return spawned and states 
        
        if return_spawned or return_states:
        
            st_ts, st_ind = self.parse_all_state_times(state='StartTrial', times='StateStarted', return_index=True)
            end_ts, end_ind = self.parse_all_state_times(state='EndTrial', times='StateStarted', return_index=True)

            if len(end_ind)>len(st_ind):
                flat= end_ind[0]
                for i in range(len(end_ind)-1):
                    flat=np.concatenate((flat,end_ind[i+1]))
                flat.sort()
                end_ind=flat
            else:
                end_ind=end_ind[0]
            ...
            if return_spawned:
                trial_info['SpawnedStimuli'] = np.empty(ntrials,dtype=object)
            if return_states:
                trial_info['States'] = np.empty(ntrials,dtype=object)


            for ist, st in enumerate(st_ind):
                        ...
                        if spawned_name in i_id['name']:
                            ...
                            if spawn_st >= st and spawn_st < end:
                                ...
                                for param in return_param:
                                    if param in stim:
                                        stim[param] = tmp[param]
                                tr_stimuli.append(stim)

                    trial_info['SpawnedStimuli'][ist]=tr_stimuli


                if return_states:
                    for i_id in trl_states:
                        tr_states.append(self.parse_stateid_times(state_id=i_id['id'], st=st, end=end))
                    trial_info['States'][ist]=tr_states
    ...
    def parse_parameters_with_trial_number(self):
        """
        Parses the log file to extract specific parameters for every tick in each trial,
        aggregating changes within the same tick.
        This method is integrated into the TextLog class.
        """
        parameter_names = []
        trial_data = {}
        current_trial = None
        last_timestamp = None
        temp_parameters = {
            'SidewardInputFraction': None,
            'ForwardInputFraction': None,
            'RotationInputFraction': None,
            'RotationSidewardFraction': None
        }

        self.mm.seek(0)  # Ensure we start from the beginning of the file
        while True:
            line = self.mm.readline().decode(self.enc)
            if not line:
                break  # End of file
            ...
            if 'LogTypes::Eventmarker' in line:
                try:
                    event_marker = int(parts[3].strip())
                    if 1000 <= event_marker < 3000:
                        current_trial = event_marker - 1000 + 1
                except ValueError:
                    pass

            elif 'LogTypes::FloatParameterHeader' in line:
                parameter_names = parts[3:]
                parameter_names = [name.strip() for name in parameter_names]

            elif 'LogTypes::FloatParameter' in line and parameter_names:
                values = parts[3:]
                values = [float(value.strip()) for value in values]
                data_dict = dict(zip(parameter_names, values))

                for param in temp_parameters.keys():
                    if param in data_dict:
                        temp_parameters[param] = data_dict[param]

            if current_trial is not None:
                if last_timestamp is None or last_timestamp != timestamp:
                    # New tick, record the parameters from the last tick
                    if current_trial not in trial_data:
                        trial_data[current_trial] = []
                    trial_data[current_trial].append({'timestamp': last_timestamp, **temp_parameters})
