"""
Color-coded raster of RF-in / RF-out status across trials for each channel in a selected array.
...
ARRAY_IDX    = 0         # 0-indexed array to analyse: 0=Array1 … 5=Array6
...
# Selected array derived from ARRAY_IDX
...
def get_good_channels(ch_indices):
    """
    Return the subset of ch_indices that have data in the HDF5 file.
    A channel is 'good' if its Point_{ch+1}_* entry exists in at least
    one time-point group across the first 10 trials.
    """
    good = []
    with h5py.File(hdf5_path, 'r') as hf:
        trial_names = sorted(hf.keys(), key=lambda n: int(n.split('_')[-1]))
...
# ============================================================
# 6. PLOT  –  two PDFs: array-level (majority vote) + per-channel
# ============================================================
...
MARKER_X_FRAC  = 0.012   # fraction of time axis added to right for outcome markers

# ---- detect good channels in selected array --------------------------------
...
with PdfPages(out_path) as pdf:
    for ch in good_channels:
        ...
        if len(gap_pos) > 0:
            # one boundary position per block gap (first gap row of each group)
        ...
        # outcome markers (right edge, outside axes)
        ...
        fig.suptitle(
            f'{session}  –  {ch_label}  –  RF in/out raster (single channel)\n'
