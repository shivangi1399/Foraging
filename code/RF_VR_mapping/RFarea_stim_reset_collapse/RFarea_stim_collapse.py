...
# -----------------------------------------------------------------------------------------#
# Geometry helpers — UNCHANGED from the original serial version
# -----------------------------------------------------------------------------------------#
...
# -----------------------------------------------------------------------------------------#
# Per-trial worker — runs on a SLURM worker, writes ONE trial to its own .h5 part file
# -----------------------------------------------------------------------------------------#
def process_trial(trial_name, input_hdf5_file, rf_path, part_dir):
    """
    Compute RF/stim overlap for a single trial and write it to its own per-trial .h5 file.
    The per-trial group structure is identical to the original single-file output, so the
    parts can be copied straight into the merged RF_stim_collapse.h5. Returns the part file
    path on success, or None if the trial failed (so the merge can skip it).
    """
    center_coords = np.load(os.path.join(rf_path, 'center_coords.npy'))
    rf_radius = np.load(os.path.join(rf_path, 'RF_radius.npy'))

    part_file = os.path.join(part_dir, f'{trial_name}.h5')
    tmp_part = part_file + '.tmp'

    try:
        with h5py.File(input_hdf5_file, 'r') as infile, h5py.File(tmp_part, 'w') as outfile:
            ...
            trial_result_group.attrs['C_Identity'] = trial_group.attrs.get('C_Identity', 'Unknown')
            ...
            for time_point_name in time_point_names:
                ...
                transformed_outline_C = time_point_group.get('transformed_outline_C', None)
                ...
                outline_C = transformed_outline_C[()] if transformed_outline_C is not None else None
                ...
                polyC = outline_C.tolist() if outline_C is not None and len(outline_C) >= 3 else []
                ...
                collapsed_C = is_collapsed(polyC, var_thresh=1.0, height_thresh=10.0, )

                collapsed_case = collapsed_A or collapsed_B or collapsed_C
                ...
                for idx, row in enumerate(center_coords):
                    ...
                    if collapsed_case:
                        inside_A, inside_B, inside_C = True, True, True
                    else:
                        ...
                        inside_C = is_overlap_at_least(center, rad, polyC) if len(polyC) >= 3 else False
                    ...
                    point_result_group.create_dataset('inside_transformed_outline_C', data=inside_C)

        os.replace(tmp_part, part_file)  # promote only once fully written
        return part_file
    except Exception as e:
...
# -----------------------------------------------------------------------------------------#
# Driver: sessions as a for loop, trials parallelised, then merge the parts
# -----------------------------------------------------------------------------------------#
if __name__ == '__main__':
    for session in sessions:
        ...
        closest_RF = min(RF_sessions, key=lambda rf: abs((RF_dates[rf] - session_date).days))
        ...
        if not os.path.exists(input_hdf5_file):
            print(f"[{session}] input not found, skipping: {input_hdf5_file}")
            continue

        # per-trial part files live in a temp subfolder, cleaned up after the merge
        part_dir = os.path.join(output_dir, 'parts_tmp')
        os.makedirs(part_dir, exist_ok=True)

        # list trials (cheap, read-only)
        with h5py.File(input_hdf5_file, 'r') as infile:
            trial_names = sorted(infile.keys(), key=lambda name: int(name.split('_')[-1]))

        n_workers = min(MAX_WORKERS, len(trial_names))
        print(f"[{session}] {len(trial_names)} trials -> {n_workers} workers on '{SLURM_PARTITION}'")

        # --- parallelise the trials across the cluster (geometry runs unchanged in process_trial) ---
        with ParallelMap(process_trial,
                         trial_names,            # distributed: one call per trial
                         input_hdf5_file,        # broadcast
                         rf_path,                # broadcast
                         part_dir,               # broadcast
                         n_inputs=len(trial_names),
                         partition=SLURM_PARTITION,
                         n_workers=n_workers,
                         mem_per_worker=MEM_PER_WORKER,
                         write_worker_results=False,   # process_trial writes its own h5; just return the path
                         setup_interactive=False) as pmap:
            part_files = pmap.compute()

        # --- merge per-trial parts into the final file (temp + rename so we never clobber) ---
        tmp_out = output_hdf5_file + '.tmp'
        n_ok = 0
        with h5py.File(tmp_out, 'w') as outfile:
            for pf in part_files:
                if pf is None or not os.path.exists(pf):
                    continue
                with h5py.File(pf, 'r') as part:
                    for tname in part.keys():
                        outfile.copy(part[tname], tname)   # fast C-level group copy
                n_ok += 1
        os.replace(tmp_out, output_hdf5_file)

        # clean up the per-trial parts
        shutil.rmtree(part_dir, ignore_errors=True)
        print(f"[{session}] merged {n_ok}/{len(trial_names)} trials -> {output_hdf5_file}")

        # sanity check: reopen the final file and confirm it is non-empty before downstream use
        with h5py.File(output_hdf5_file, 'r') as check:
            n_written = len(check.keys())
        if n_written == 0:
            print(f"[{session}] WARNING: final file has 0 trials -- nothing was written!")
        elif n_written != len(trial_names):
            print(f"[{session}] WARNING: final file has {n_written}/{len(trial_names)} trials "
                  f"({len(trial_names) - n_written} missing/failed)")
        else:
            print(f"[{session}] OK: final file has all {n_written} trials")
