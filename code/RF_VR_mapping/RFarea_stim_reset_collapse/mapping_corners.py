...
for session in sessions:
    ...
    # get stimulus path for each trial -----------------------------------------------------------------------
    stim_loc = []
    stim_ts = []
    stim_params = []
    ...
    for csv_file in csv_files:
        ...
        if 'net.csv' not in basename:
            eye_file = basename.replace('.csv', '')
    ...
    for ist, st in enumerate(trl_starts):
        ...
        eye_trl_evt.append(eye_evt[inc_evt, :]) #event data for the duration the leaf is on
        
    # convert stimulus to eye coordinates ------------------------------------------------------------------------------
    def find_stimulus_corners(azimuth, elevation, radius, width = 200, height = 200, vertical_offset = 0):
    ...
    for itrl, irec_xy in enumerate(eye_trl): #gives spherical coordinates of the four corners of both the stimuli
        itrl
        x, y, z = stim_loc[itrl * n_stimuli + 2].T #x, y and z is a series of locations with time for the trial
        dome_x1, dome_y1 = find_stimulus_corners(azimuth = x,
                                                elevation = y,
                                                radius = z,
                                                width = stim_width[itrl * n_stimuli + 2],
                                                height = stim_width[itrl * n_stimuli + 2],
                                                vertical_offset = vertical_offset[itrl * n_stimuli + 2])
        
        x, y, z = stim_loc[itrl * n_stimuli + 3].T 
        ...
        x, y, z = stim_loc[itrl * n_stimuli + 4].T #reset button
        dome_x3, dome_y3 = find_stimulus_corners(azimuth = x,
                                                elevation = y,
                                                radius = z,
        ...
        eccentricity, polar = dc.dome2eye(dome_x = np.concatenate((dome_x1.T, dome_x2.T, dome_x3.T), axis = 1),
                                            dome_y = np.concatenate((dome_y1.T, dome_y2.T, dome_y3.T), axis = 1),
                                            irec_x = irec_xy[:, 0],
                                            irec_y = irec_xy[:, 1],
    ...
    # converting stimuli coordinates from retinal to cartesian -------------------------------------------------
    # to check if RF lies inside meshes

    def retinal2cartesian(eccentricity, polar, R=1): 
    ...
    for trial_polar, trial_eccentricity in zip(polar_trl, eccentricity_trl):
        ...
        for time_point_polar, time_point_eccentricity in zip(trial_polar, trial_eccentricity):
            stimulus1_polar = time_point_polar[:4]  # This goes into a trial in polar_trl and eccentricity_trl and in one time point and takes 
                                                    # the first 4 coordinates, which are the 4 corners of mesh 1. 
                                                    # For example trial 1, time point 1 is (polar_trl[0][0][:4]). Same for eccentricity_trl
            stimulus1_eccentricity = time_point_eccentricity[0:4]
            
            stimulus2_polar = time_point_polar[4:8]
            stimulus2_eccentricity = time_point_eccentricity[4:8]

            stimulus3_polar = time_point_polar[8:12]
            stimulus3_eccentricity = time_point_eccentricity[8:12]
            ...
            # Convert stimulus 3 coordinates. 
            stimulus3_coords = [retinal2cartesian(e, p) for e, p in zip(stimulus3_eccentricity, stimulus3_polar)]
            
            # Combine both stimuli coordinates for the time point
            time_point_coords = stimulus1_coords + stimulus2_coords + stimulus3_coords
    ...
    print(f"Length of cartesian_trials: {len(cartesian_trials)}")  
    ...
    # if you want to get the 4 corners of mesh 1 for trial 1, time point 1:  cartesian_trials[0][0][:4]  and for
    # mesh 2: cartesian_trials[0][0][4:8] and for mesh 3: cartesian_trials[0][0][8:12]

    # Clear memory after finishing this session
    del stim_loc, stim_ts, stim_params, stim_width, vertical_offset
    del eye_trl, eye_trl_ts, eye_trl_evt
    del eccentricity_trl, polar_trl, cartesian_trials
    plt.close('all')
    gc.collect()
